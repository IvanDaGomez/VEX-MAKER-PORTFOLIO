...
void autonomous(void) {
 ...
 driveL.spinFor(forward,0.4,turns);
 DigitalOutB.set(false);
 Brazodelantero.spinFor(forward, 0.05, turns);
 alldrive.setVelocity(100,percent);
 driveR.spinFor(reverse,1,turns);
 alldrive.spinFor(forward, 2, turns);
 alldrive.setVelocity(50, percent);
 driveL.spinFor(forward,1, turns) && driveR.spinFor(reverse, 1, turns);
 alldrive.spinFor(reverse,0.4, turns);
 Brazotrasero.spin(forward);
 wait(0.5,sec);
 Brazotrasero.stop();
 alldrive.spinFor(forward,1.5, turns);
}
